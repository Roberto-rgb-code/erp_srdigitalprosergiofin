...
@section('content')
    ...
    <form action="{{ route('servicios_empresariales.store') }}" method="POST">
        ...
        <div class="mb-3">
            <label for="poliza" class="form-label">Póliza asociada (texto libre)</label>
            <input type="text" name="poliza" id="poliza" class="form-control" value="{{ old('poliza') }}" placeholder="Escribe el nombre, tipo o folio de la póliza">
            <small class="text-muted">Puedes escribir el nombre, tipo o referencia de la póliza.</small>
        </div>
